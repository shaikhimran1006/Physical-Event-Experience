...
export default function OverviewPage({ zones, queues, interventions }) {
  const stats = useMemo(() => {
    ...
    const gateQueues = Object.values(queues).filter(
      (item) => item.point_type === "gate",
    );

    const totalOccupancy = zoneValues.reduce(
      (sum, zone) => sum + (zone.currentOccupancy || 0),
      0,
    );
    const totalCapacity = zoneValues.reduce(
    ...
    const avgGateWait = gateQueues.length
      ? (
          gateQueues.reduce(
            (sum, item) => sum + (item.avgWaitMinutes || 0),
            0,
          ) / gateQueues.length
        ).toFixed(1)
      : "0.0";
    ...
    return {
      ...
      totalCapacity,
      avgGateWait,
      activeAlerts: interventions.filter((item) => item.status === "pending")
        .length,
      hotZones: zoneValues.filter((zone) =>
        ["red", "orange"].includes(zone.status),
      ).length,
    };
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Real-time venue intelligence and intervention control
          </p>
        </div>
      </div>
          ...
          .map(([zoneId, zone]) => (
            <div key={zoneId} className="card zone-card">
              ...
              >
                ...
                <span
                  style={{
                    color: `var(--status-${zone.status || "green"})`,
                    fontSize: "0.85rem",
                  }}
                >
                  <i className={statusIcon(zone.status)} aria-hidden="true" />{" "}
              ...
              <div className="zone-capacity">
                Capacity: {(zone.capacity || 0).toLocaleString()}
              </div>
            </div>
          ))}
      </div>
