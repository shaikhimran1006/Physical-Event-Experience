...
const API_URL = normalizeBaseUrl(runtimeConfig.API_URL || import.meta.env.VITE_API_URL || 'http://localhost:8000');
const WS_URL = normalizeBaseUrl(runtimeConfig.WS_URL || import.meta.env.VITE_WS_URL || API_URL.replace(/^http/i, 'ws'));
...
function SimulationWidget() {
  ...
  const startSim = async () => {
    ...
    try {
      await fetch(`${API_URL}/simulation/start`, {
        ...
        body: JSON.stringify({ mode: 'demo', speed_factor: 10, venue_id: 'stadium_01' }),
      });
    } catch (err) {
  ...
  const phaseEmojis = {
    pre_game: '🏟️', in_game: '⚽', in_game_2: '⚽',
    halftime: '🍕', post_game: '🚶', idle: '💤',
    completed: '✅', starting: '🚀', stopped: '⏹️',
  };
  ...
  return (
    <div className="card" style={{ padding: '12px' }}>
      <div className="flex items-center justify-between mb-md">
        ...
        <span className="sim-phase">{phaseEmojis[simStatus.phase] || '💤'} {simStatus.phase.replace(/_/g, ' ')}</span>
      </div>

      {simStatus.running && (
        <div className="sim-progress-wrapper" style={{ marginBottom: '12px' }}>
          <div className="sim-progress-bar" style={{ width: `${simStatus.progress}%` }} />
        </div>
      )}

      <div className="flex gap-sm">
        {!simStatus.running ? (
          <button className="btn btn-primary" style={{ flex: 1, fontSize: '0.75rem' }} onClick={startSim} disabled={loading} id="btn-start-sim">
            {loading ? '⏳ Starting...' : '▶ Start Demo'}
          </button>
        ) : (
          <button className="btn btn-danger" style={{ flex: 1, fontSize: '0.75rem' }} onClick={stopSim} id="btn-stop-sim">
            ⏹ Stop
          </button>
        )}
      </div>
...
function OverviewPage({ zones, queues, interventions }) {
  const totalOccupancy = Object.values(zones).reduce((s, z) => s + (z.currentOccupancy || 0), 0);
  const totalCapacity = Object.values(zones).reduce((s, z) => s + (z.capacity || 5000), 0);

  const gates = Object.entries(queues).filter(([, v]) => v.point_type === 'gate');
  const avgGateWait = gates.length > 0
    ? (gates.reduce((s, [, v]) => s + (v.avgWaitMinutes || 0), 0) / gates.length).toFixed(1)
    : '0.0';

  const activeAlerts = interventions.filter(i => i.status === 'pending').length;
  const redZones = Object.values(zones).filter(z => z.status === 'red' || z.status === 'orange').length;
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Real-time venue intelligence — MetLife Stadium</p>
        </div>
      ...
      <div className="stats-grid">
        <div className="card stat-card accent-blue">
          ...
          <div className="card-value text-blue">{totalOccupancy.toLocaleString()}</div>
          <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>of {totalCapacity.toLocaleString()} capacity</div>
        ...
        <div className="card stat-card accent-amber">
          ...
          <div className="card-value" style={{ color: 'var(--accent-amber)' }}>{avgGateWait}</div>
          <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>minutes</div>
        </div>
        ...
        <div className="card stat-card accent-green">
          ...
          <div className="card-value" style={{ color: redZones > 0 ? 'var(--status-orange)' : 'var(--status-green)' }}>
            {redZones}
          </div>
          <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>of {Object.keys(zones).length} zones</div>
        </div>
      </div>

      <div className="two-col-layout">
        <div>
          <h2 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: '16px' }}>🗺️ Zone Status</h2>
          <div className="heatmap-grid" style={{ gridTemplateColumns: 'repeat(2, 1fr)' }}>
            {Object.entries(zones).slice(0, 8).map(([zoneId, zone]) => (
        ...
        <div>
          <h2 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: '16px' }}>🚨 Latest Interventions</h2>
          <div className="alerts-container">
            {interventions.slice(0, 4).map((alert, i) => (
              <AlertCard key={alert.intervention_id || i} alert={alert} compact />
            ))}
...
function ZoneCard({ zoneId, zone, compact }) {
  ...
  return (
    <div className="card zone-card" id={`zone-${zoneId}`}>
      ...
      <div className="zone-occupancy" style={{ color: `var(--status-${status})` }}>
      ...
      <div className="zone-bar">
        <div className={`zone-bar-fill ${status}`} style={{ width: `${Math.min(100, pct)}%` }} />
      </div>
    </div>
  );
}
...
function StadiumMapPage({ zones, queues }) {
  ...
  const getStatusGlow = (status) => {
    const colors = {
      green: 'rgba(34,197,94,0.3)', yellow: 'rgba(234,179,8,0.3)',
      orange: 'rgba(249,115,22,0.4)', red: 'rgba(239,68,68,0.5)',
    };
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Real-time 2D venue visualization with live congestion data</p>
        </div>
        <div className="map-legend">
          <span className="legend-item"><span className="legend-dot" style={{ background: '#22c55e' }} /> Clear</span>
          <span className="legend-item"><span className="legend-dot" style={{ background: '#eab308' }} /> Busy</span>
          <span className="legend-item"><span className="legend-dot" style={{ background: '#f97316' }} /> Crowded</span>
      ...
      <div className="stadium-map-container">
        <svg viewBox="-10 -10 120 120" className="stadium-map-svg" xmlns="http://www.w3.org/2000/svg">
          ...
          <ellipse cx="50" cy="50" rx="48" ry="48" fill="none" stroke="rgba(255,255,255,0.08)" strokeWidth="0.5" />
          <ellipse cx="50" cy="50" rx="44" ry="44" fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="12" />

          {/* Playing field */}
          <ellipse cx="50" cy="50" rx="28" ry="20" fill="url(#fieldGrad)" stroke="#22c55e" strokeWidth="0.3" opacity="0.6" />
          {/* Field markings */}
          <line x1="50" y1="30" x2="50" y2="70" stroke="#22c55e" strokeWidth="0.2" opacity="0.4" />
          <ellipse cx="50" cy="50" rx="6" ry="4" fill="none" stroke="#22c55e" strokeWidth="0.2" opacity="0.4" />
          ...
          {Object.entries(ZONE_POSITIONS).map(([zoneId, pos]) => {
            ...
            const angle = {
              zone_A1: -90, zone_A2: -90,
              zone_B1: 0, zone_B2: 0,
              zone_C1: 90, zone_C2: 90,
              zone_D1: 180, zone_D2: 180,
            }[zoneId] || 0;
            ...
            return (
              <g key={zoneId} className="map-zone-group">
                <circle
                  cx={cx} cy={cy} r={isInner ? 5 : 4}
                  fill={color} opacity={0.25}
                  stroke={color} strokeWidth="0.5"
                  filter="url(#glow)"
                />
                <circle
                  cx={cx} cy={cy} r={isInner ? 3.5 : 2.8}
                  fill={color} opacity={0.6 + pct * 0.004}
                  className={status === 'red' ? 'pulse-zone' : ''}
                />
                <text x={cx} y={cy - (isInner ? 7 : 6)} textAnchor="middle" fill="white" fontSize="2.2" fontWeight="700" opacity="0.9">
                  {zoneId.replace('zone_', '').toUpperCase()}
                </text>
                <text x={cx} y={cy + 1} textAnchor="middle" fill="white" fontSize="1.8" fontWeight="800" fontFamily="'JetBrains Mono', monospace">
                  {pct}%
                </text>
              </g>
            );
          ...
          {Object.entries(GATE_POSITIONS).map(([gateId, pos]) => {
            ...
            const angle = { gate_A: -90, gate_B: 0, gate_C: 90, gate_D: 180 }[gateId] || 0;
            const rad = (angle * Math.PI) / 180;
            ...
            return (
              <g key={gateId}>
                <rect x={cx - 4} y={cy - 2.5} width="8" height="5" rx="1.5"
                  fill={color} opacity="0.8" stroke="white" strokeWidth="0.3" />
                <text x={cx} y={cy + 0.8} textAnchor="middle" fill="white" fontSize="2" fontWeight="700">
                  {wait.toFixed(0)}m
                </text>
                <text x={cx} y={cy - 3.5} textAnchor="middle" fill={color} fontSize="2" fontWeight="700">
          ...
          {/* Center label */}
          <text x="50" y="50" textAnchor="middle" fill="rgba(255,255,255,0.3)" fontSize="3" fontWeight="700">
            FIELD
          </text>
        </svg>
        ...
        <div className="map-stats-overlay">
          <div className="map-stat">
            ...
            <div className="map-stat-value">
              {Object.values(zones).reduce((s, z) => s + (z.currentOccupancy || 0), 0).toLocaleString()}
            </div>
          </div>
          <div className="map-stat">
            <div className="map-stat-label">Hotspots</div>
            <div className="map-stat-value" style={{ color: 'var(--status-orange)' }}>
              {Object.values(zones).filter(z => z.status === 'red' || z.status === 'orange').length}
            </div>
          </div>
          <div className="map-stat">
            ...
            <div className="map-stat-value">
              {(() => {
                const g = Object.values(queues).filter(q => q.point_type === 'gate');
                return g.length > 0 ? (g.reduce((s, q) => s + (q.avgWaitMinutes || 0), 0) / g.length).toFixed(1) : '0';
              })()}m
            </div>
          </div>
        </div>
      ...
      <h2 style={{ fontSize: '1rem', fontWeight: 700, margin: '24px 0 12px' }}>All Zones</h2>
      <div className="heatmap-grid">
        {Object.entries(zones).map(([zoneId, zone]) => (
          <ZoneCard key={zoneId} zoneId={zoneId} zone={zone} />
        ))}
        {Object.keys(zones).length === 0 && (
          <div className="card" style={{ gridColumn: '1 / -1', textAlign: 'center', color: 'var(--text-muted)', padding: '60px' }}>
...
function QueuesPage({ queues }) {
  const gates = Object.entries(queues).filter(([, v]) => v.point_type === 'gate');
  const concessions = Object.entries(queues).filter(([, v]) => v.point_type === 'concession');
  const restrooms = Object.entries(queues).filter(([, v]) => v.point_type === 'restroom');
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Real-time queue lengths and wait time predictions</p>
        </div>
      </div>
...
function Sparkline({ data, color, width = 60, height = 20 }) {
  ...
  return (
    <svg width={width} height={height} className="sparkline-svg" style={{ display: 'block' }}>
      <polyline
        points={points}
        fill="none"
      ...
      <defs>
        <linearGradient id={`sparkFill-${color}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={color || 'var(--accent-blue)'} stopOpacity="0.3" />
          <stop offset="100%" stopColor={color || 'var(--accent-blue)'} stopOpacity="0" />
        </linearGradient>
      </defs>
      <polyline
...
function QueueCard({ pointId, data }) {
  ...
  if (prediction && prediction.predictions) {
    ...
    if (pred15) {
      ...
      const cls = predWait > wait * 1.5 ? 'danger' : predWait > wait ? 'warning' : '';
      predLabel = (
    ...
    <div className="card queue-card" id={`queue-${pointId}`}>
      ...
      <div className="queue-info">
        <div className="flex items-center justify-between">
          <div className="queue-name">{pointId.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</div>
          <Sparkline data={historyRef.current} color={statusColors[status]} width={60} height={18} />
        </div>
        <div className="queue-metrics">
          <div className="queue-metric">
            <div className="queue-metric-label">Wait</div>
            <div className="queue-metric-value" style={{ color: `var(--status-${status})` }}>
              {wait.toFixed(1)}<span style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}> min</span>
            </div>
          </div>
          <div className="queue-metric">
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">AI-generated recommendations — approve or dismiss</p>
        </div>
        <div className="flex gap-sm">
      ...
      <div className="alerts-container">
        ...
        {interventions.length === 0 && (
          <div className="card" style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '60px' }}>
            No interventions generated yet — the system will create them when congestion thresholds are exceeded
          </div>
        )}
      </div>
    </div>
...
function AlertCard({ alert, compact }) {
  ...
  const handleAction = async (action) => {
    try {
      const res = await fetch(`${API_URL}/interventions/${alert.intervention_id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className={`card alert-card ${severity} ${actionState !== 'pending' ? 'resolved' : ''}`} id={`alert-${alert.intervention_id || ''}`}>
      <div className="alert-header">
        ...
        <div className="flex items-center gap-sm">
          {actionState !== 'pending' && (
            <span className={`alert-badge ${actionState === 'approved' ? 'approved' : 'dismissed'}`}>
      ...
      {!compact && actionState === 'pending' && (
        <div className="alert-actions">
          <button className="btn btn-success" onClick={() => handleAction('approve')} id={`approve-${alert.intervention_id || ''}`}>
            ✓ Approve
          </button>
          <button className="btn btn-ghost" onClick={() => handleAction('dismiss')} id={`dismiss-${alert.intervention_id || ''}`}>
            ✕ Dismiss
          </button>
        </div>
      )}
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Before vs. After — measurable impact of Stadium OS Copilot</p>
        </div>
      </div>
        ...
        <>
          <div className="stats-grid" style={{ marginBottom: '32px' }}>
            <div className="card stat-card accent-green">
              ...
              <div className="card-value text-green">{kpis.improvements?.avg_wait_reduction_pct || 0}%</div>
              <div className="stat-trend down">↓ Lower is better</div>
            </div>
            <div className="card stat-card accent-blue">
              <div className="card-title">P95 Wait Reduction</div>
              <div className="card-value text-blue">{kpis.improvements?.p95_wait_reduction_pct || 0}%</div>
              <div className="stat-trend down">↓ Tail latency crushed</div>
            </div>
            <div className="card stat-card accent-amber">
              <div className="card-title">Response Speed</div>
              <div className="card-value" style={{ color: 'var(--accent-amber)' }}>{kpis.improvements?.response_improvement_factor || 'N/A'}</div>
              <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>faster than human-only</div>
            </div>
            <div className="card stat-card accent-red">
              <div className="card-title">Copilot Response</div>
              <div className="card-value text-green">{kpis.improvements?.response_latency_copilot_sec || 0}s</div>
              <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>vs {kpis.improvements?.response_latency_baseline_min || 10}min baseline</div>
            </div>
          </div>
          ...
          {kpis.improvements?.hotspot_baseline_pct > 0 && (
            <div className="stats-grid" style={{ marginBottom: '32px', gridTemplateColumns: 'repeat(3, 1fr)' }}>
              <div className="card stat-card accent-red">
                ...
                <div className="card-value text-red">{kpis.improvements.hotspot_baseline_pct}%</div>
                <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>of time in congestion</div>
              </div>
              <div className="card stat-card accent-green">
                <div className="card-title">Hotspot Time (Copilot)</div>
                <div className="card-value text-green">{kpis.improvements.hotspot_copilot_pct}%</div>
                <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>70% reduction via rerouting</div>
              </div>
              <div className="card stat-card accent-blue">
                <div className="card-title">Interventions Generated</div>
                <div className="card-value text-blue">{kpis.improvements.interventions_generated || 0}</div>
                <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>automated AI actions</div>
              </div>
            </div>
          )}
            ...
            <div className="kpi-comparison">
              <div className="kpi-column before">
                ...
                <div className="kpi-item">
                  ...
                  <span className="kpi-value text-red">{kpis.baseline?.avg_wait_minutes || 0} min</span>
                </div>
                <div className="kpi-item">
                  <span className="kpi-label">P95 Wait</span>
                  <span className="kpi-value text-red">{kpis.baseline?.p95_wait_minutes || 0} min</span>
                </div>
                <div className="kpi-item">
                  <span className="kpi-label">Max Wait</span>
                  <span className="kpi-value text-red">{kpis.baseline?.max_wait_minutes || 0} min</span>
                </div>
                <div className="kpi-item">
                  <span className="kpi-label">Response Time</span>
                  <span className="kpi-value text-red">{kpis.improvements?.response_latency_baseline_min || 10} min</span>
                </div>
              </div>
              ...
              <div className="kpi-column after">
                ...
                <div className="kpi-item">
                  <span className="kpi-label">Avg Wait</span>
                  <span className="kpi-value text-green">{kpis.optimized?.avg_wait_minutes || 0} min</span>
                </div>
                <div className="kpi-item">
                  <span className="kpi-label">P95 Wait</span>
                  <span className="kpi-value text-green">{kpis.optimized?.p95_wait_minutes || 0} min</span>
                </div>
                <div className="kpi-item">
                  <span className="kpi-label">Max Wait</span>
                  <span className="kpi-value text-green">{kpis.optimized?.max_wait_minutes || 0} min</span>
                </div>
                <div className="kpi-item">
                  <span className="kpi-label">Response Time</span>
                  <span className="kpi-value text-green">{kpis.improvements?.response_latency_copilot_sec || 0} sec</span>
                </div>
              </div>
            </div>
          </div>
...
export default function App() {
  ...
  const { zones, queues, interventions, kpis, connected, wsConnected } = useVenueData();

  const renderPage = () => {
    switch (activePage) {
      case 'heatmap': return <StadiumMapPage zones={zones} queues={queues} />;
      case 'queues': return <QueuesPage queues={queues} />;
      case 'alerts': return <AlertsPage interventions={interventions} />;
      case 'kpis': return <KPIsPage kpis={kpis} />;
      default: return <OverviewPage zones={zones} queues={queues} interventions={interventions} />;
    }
  };
  ...
  return (
    <div className="app-layout" id="ops-dashboard">
      ...
      <main className="main-content">
        {!connected && (
          <div className="sim-controls" style={{ borderColor: 'rgba(239, 68, 68, 0.3)' }}>
            <span style={{ color: 'var(--status-red)', fontSize: '0.85rem' }}>
              ⚠️ Cannot connect to backend at {API_URL} — make sure the FastAPI server is running
            </span>
          </div>
        )}
        {connected && (
          <div className="connection-badge" style={{ position: 'fixed', top: '12px', right: '16px', zIndex: 999 }}>
